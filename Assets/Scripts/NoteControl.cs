...
public class NoteControl : MonoBehaviour {
	...
	void Update () {
	    if (( songControl.destroyD == "y") && (gameObject.name == "bombD(Clone)") && (transform.position.y<=-1.3))
        {
        ...
        if ((songControl.destroyF == "y") && (gameObject.name == "bombF(Clone)") && (transform.position.y <= -1.3))
        {
            //Instantiate(burst, transform.position, burst.rotation);
            songControl.destroyF = "n";
            Destroy(gameObject);
            addToStreak();
            player.points += points * player.multiplier;
        }
        if ((songControl.destroyG == "y") && (gameObject.name == "bombG(Clone)") && (transform.position.y <= -1.3))
        {
            //Instantiate(burst, transform.position, burst.rotation);
            songControl.destroyG = "n";
            Destroy(gameObject);
            addToStreak();
            player.points += points * player.multiplier;
        }
